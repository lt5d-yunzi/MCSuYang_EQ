...
namespace zlgui::dragger {
    ...
    void Dragger::mouseDown(const juce::MouseEvent& e) {
        ...
        is_dragging_horizontally_ = false;
    ...
    void Dragger::mouseDrag(const juce::MouseEvent& e) {
        if (std::abs(e.getOffsetFromDragStart().x) > 4) {
            is_dragging_horizontally_ = true;
        }
        ...
        // update button position synchronously
        updateButton(current_pos_);
        // call listeners
        const BailOutChecker checker(this);
