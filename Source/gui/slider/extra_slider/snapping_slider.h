...
namespace zlgui::slider {
    ...
    public:
        ...
        void mouseDown(const juce::MouseEvent& event) override {
            if (!is_track_click_enabled_) {
                if (event.mods.isPopupMenu()) {
                    juce::Slider::mouseDown(event);
                    return;
                }
                if (is_on_thumb(event.position)) {
                    juce::Slider::mouseDown(event);
                }
            } else {
                juce::Slider::mouseDown(event);
        ...
        void setTrackClickEnabled(bool enabled) {
            is_track_click_enabled_ = enabled;
        }
    ...
    protected:
        UIBase& base_;
        bool is_track_click_enabled_{true};
        double value_on_mouse_down_{0.0};

        bool is_on_thumb(juce::Point<float> pos) {
            auto style = getSliderStyle();
            if (style == juce::Slider::LinearHorizontal) {
                float thumbX = static_cast<float>(getPositionOfValue(getValue()));
                return std::abs(pos.x - thumbX) <= 15.0f;
            } else if (style == juce::Slider::LinearVertical) {
                float thumbY = static_cast<float>(getPositionOfValue(getValue()));
                return std::abs(pos.y - thumbY) <= 15.0f;
            }
            return true;
        }
