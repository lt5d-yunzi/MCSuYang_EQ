    ...
    public:
        ...
        void drawComboBox(juce::Graphics& g, int width, int height, bool isButtonDown, int, int, int, int,
            ...
            if (isButtonDown || box.isPopupActive()) {
                g.setColour(base_.getBackgroundInactiveColour());
                g.fillRoundedRectangle(box_bound, corner_size);
                g.setColour(base_.getTextColour().withAlpha(0.2f));
                g.drawRoundedRectangle(box_bound, corner_size, 1.0f);
            ...
            if (!icons_.empty() && box.getSelectedItemIndex() >= 0) {
                ...
                const auto edge_indent = base_.getFontSize() * .1575f;
                fig->drawWithin(g, box.getLocalBounds().toFloat().reduced(edge_indent), juce::RectanglePlacement::centred, 1.f);
            }
        }
        ...
        void drawLabel(juce::Graphics& g, juce::Label& label) override {
            ...
            auto font = juce::Font(base_.getFontSize() * font_scale_ * 1.15f);
            font.setBold(true);
            g.setFont(font);
        ...
        void drawPopupMenuItem(juce::Graphics& g, const juce::Rectangle<int>& area,
            ...
            if (icon == nullptr) {
                ...
                fig->replaceColour(juce::Colours::black, base_.getTextColour());
                const auto edge_indent = base_.getFontSize() * .1575f;
                fig->drawWithin(g, area.toFloat().reduced(edge_indent), juce::RectanglePlacement::centred, alpha);
            }
        }
