...
namespace zlpanel {
    ...
    void FFTPanel::paint(juce::Graphics& g) {
        ...
        if (is_on[0]) {
            g.setColour(base_.getColourByIdx(zlgui::ColourIdx::kPreColour).withAlpha(0.12f));
            g.fillPath(paths_[0].getReader());
        }
        if (is_on[1]) {
            const auto thickness = base_.getFontSize() * .12f;
            g.setColour(base_.getColourByIdx(zlgui::ColourIdx::kPostColour).withAlpha(0.5f));
            g.strokePath(paths_[1].getReader(),
                         {thickness, juce::PathStrokeType::curved, juce::PathStrokeType::rounded});
            g.setColour(base_.getColourByIdx(zlgui::ColourIdx::kPostColour).withAlpha(0.15f));
            g.fillPath(paths_[1].getReader());
        }
        if (is_on[2]) {
            g.setColour(base_.getColourByIdx(zlgui::ColourIdx::kSideColour).withAlpha(0.1f));
