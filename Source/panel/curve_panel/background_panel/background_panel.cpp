...
namespace zlpanel {
    ...
    void BackgroundPanel::drawFreqs(juce::Graphics& g) const {
        ...
        const auto top_bar_height = base_.getFontSize() * 1.4f;
        const auto bottom_bar_height = base_.getFontSize() * 1.4f;
        ...
        const auto top_solid_stop = (top_bar_height + base_.getFontSize() * 1.3f) / bound.getHeight();
        const auto top_fade_stop = (top_bar_height + base_.getFontSize() * 1.6f) / bound.getHeight();
        const auto bottom_solid_stop = (bound.getBottom() - bottom_bar_height) / bound.getHeight();
        const auto bottom_fade_stop = (bound.getBottom() - bottom_bar_height - base_.getFontSize() * 0.3f) / bound.getHeight();

        gradient.addColour(0.0, base_.getBackgroundColour().withAlpha(1.f));
        gradient.addColour(top_solid_stop, base_.getBackgroundColour().withAlpha(1.f));
        gradient.addColour(top_fade_stop, base_.getBackgroundColour().withAlpha(0.f));
        gradient.addColour(bottom_fade_stop, base_.getBackgroundColour().withAlpha(0.f));
        gradient.addColour(bottom_solid_stop, base_.getBackgroundColour().withAlpha(1.f));
        gradient.addColour(1.0, base_.getBackgroundColour().withAlpha(1.f));

        g.setGradientFill(gradient);
        g.fillRect(getLocalBounds());

        // draw freq labels (top only)
        g.setColour(base_.getTextColour().withAlpha(.5f));
        g.setFont(base_.getFontSize() * 1.25f);
        const auto top_label_y0 = top_bar_height + base_.getFontSize() * 0.15f;
        ...
        for (const auto& freq : kFreqValues) {
            ...
            const auto top_rect = juce::Rectangle(static_cast<float>(p) * bound.getWidth() - label_width * .5f, top_label_y0,
                                                  label_width, label_height);
            
            if (top_rect.getRight() > full_width) {
                break;
            }
            g.drawText(label, top_rect, juce::Justification::centredTop, false);
        }
    }
