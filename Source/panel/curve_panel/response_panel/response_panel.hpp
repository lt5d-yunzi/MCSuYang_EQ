...
namespace zlpanel {
    ...
    public:
        ...
        float getSelectedBandX() const {
            if (const auto band = base_.getSelectedBand(); band < zlp::kBandNum) {
                return points_[band][0].load(std::memory_order::relaxed);
            }
            return -1.f;
        }
    ...
    private:
        ...
        std::array<zldsp::filter::Ideal<float, 1>, zlp::kBandNum> precise_ideal_{};
        std::array<std::array<zldsp::filter::Ideal<float, 1>, zlp::Controller::kMaxPreciseSubBands>, zlp::kBandNum> precise_sub_ideal_{};
        ...
        // Per-band center frequency injection info (for accurate curve rendering)
        // Multi-point injection: replace center ± kInjectHalf grid points with exact filter values
        static constexpr int kInjectHalf = 2;
        static constexpr double kInjectStep = 0.5; // fraction of grid step between injected points
        std::array<int, zlp::kBandNum> band_center_idx_{}; // nearest grid index (-1 = uninitialized)
        std::array<float, zlp::kBandNum> band_center_w_{}; // exact center angular frequency
        std::array<float, zlp::kBandNum> band_center_x_{}; // exact center x pixel position
        std::array<float, zlp::kBandNum> band_center_idx_frac_{}; // fractional grid index (for multi-point xs computation)
