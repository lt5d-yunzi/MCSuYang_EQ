...
namespace zlpanel::band_helper {
    ...
    inline void turnOnOffDynamic(PluginProcessor& p, const size_t c_band, const bool dynamic_on) {
        ...
        if (dynamic_on) {
            ...
            const auto target_gain = getValue(p.parameters_, zlp::PTargetGain::kID + band_s);
            if (std::abs(target_gain - gain) < 0.01f) {
                ...
                updateValue(target_gain_para,
                            target_gain_para->convertTo0to1(gain - eq_max_db * .33f));
            }
        }
