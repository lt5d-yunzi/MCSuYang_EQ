...
namespace zlp {
    FilterAttach::FilterAttach(juce::AudioProcessor& processor,
        ...
        last_gain_(gain_.load(std::memory_order::relaxed)),
    ...
    void FilterAttach::parameterChanged(const juce::String& parameter_ID, const float value) {
        if (parameter_ID.startsWith(PFilterStatus::kID)) {
            ...
            empty_.setFilterType(choiceIndexToFilterType(static_cast<int>(std::round(value))));
            update_flag_.signal();
            ...
            auto& pluginProcessor = static_cast<PluginProcessor&>(processor_);
            if (!pluginProcessor.is_restoring_or_loading_) {
                const float diff = value - last_gain_;
                auto* target_gain_param = parameters_.getParameter(PTargetGain::kID + std::to_string(idx_));
                if (target_gain_param != nullptr) {
                    const float current_target_gain = target_gain_param->convertFrom0to1(target_gain_param->getValue());
                    const float new_target_gain = std::clamp(current_target_gain + diff, -30.0f, 30.0f);
                    target_gain_param->setValueNotifyingHost(target_gain_param->convertTo0to1(new_target_gain));
                }
            }
            last_gain_ = value;
