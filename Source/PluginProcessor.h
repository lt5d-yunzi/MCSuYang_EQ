...
public:
    ...
    std::atomic<float> output_peak_l_{0.0f};
    std::atomic<float> output_peak_r_{0.0f};
    bool is_restoring_or_loading_{false};
    ...
    void toggleABState();
    void copyActiveStateToInactive();
    bool isStateBActive() const { return is_state_B_active_; }
    void loadPreset(int preset_index);

private:
    juce::ValueTree state_A_;
    juce::ValueTree state_B_;
    bool is_state_B_active_{false};
